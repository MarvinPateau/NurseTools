      ...
      <div className="grid md:grid-cols-2 gap-6 items-center">
        <div className="order-2 md:order-1">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold leading-tight mt-2">
            Calculs rapides, scores cliniques & repères utiles
          </h2>
          <p className="text-slate-600 mt-3">
            Tout en français, pensé pour gagner de précieuses minutes : doses,
            débits, gouttes/min, GCS, NEWS2, clairance de la créatinine, IMC… et
            plus.
          </p>
          <div className="mt-4 text-xs text-slate-500">
            ⚠️ Aide au calcul uniquement. Double contrôle recommandé.
          </div>
        </div>
...
function Result({ children, tone = 'ok' }: ResultProps) {
  const toneMap = {
...
function DoseCalculator() {
  ...
  const [voulu, setVoulu] = useState(100);
  const [dispo, setDispo] = useState(250);
  const [contenuAmpoule, setContenuAmpoule] = useState(1000);
  const [volumeAmpoule, setVolumeAmpoule] = useState(10);
  const [doseSouhaitee, setDoseSouhaitee] = useState(250);
  ...
  const res = useMemo(() => {
    if (mode === 'mgkg') {
      ...
      return {
        text: `${round(doseTotaleMg)} mg au total → ${round(
          ml
        )} mL à prélever.`,
        tone: 'ok',
      };
    ...
    if (mode === 'regle3') {
      ...
      return { text: `${round(ml)} mL à prélever.`, tone: 'ok' };
    }
    if (mode === 'dilution') {
      const v1 =
        safeDiv(Number(doseSouhaitee), Number(contenuAmpoule)) *
        Number(volumeAmpoule);
      return {
        text: `${round(
          v1
        )} mL à prélever depuis l'ampoule. Compléter avec solvant selon protocole.`,
        tone: 'info',
      };
    }
  ...
  return (
    <Card title="Calcul de dose" subtitle="Règle de trois, mg/kg, dilution">
      ...
      {mode === 'mgkg' && (
        <div>
          <Field
            ...
            value={poids}
            onChange={(value) => setPoids(Number(value))}
            suffix="kg"
            step="0.1"
          />
          <Field
            ...
            value={doseMgKg}
            onChange={(value) => setDoseMgKg(Number(value))}
            suffix="mg/kg"
            step="0.1"
          />
          <Field
      ...
      {mode === 'regle3' && (
        <div>
          <Field
            label="Dose voulue"
            value={voulu}
            onChange={(value) => setVoulu(Number(value))}
            suffix="mg"
            step="0.1"
          />
          <Field
            ...
            value={dispo}
            onChange={(value) => setDispo(Number(value))}
            suffix="mg/mL"
            step="0.1"
          />
          <Result>{res.text}</Result>
      ...
      {mode === 'dilution' && (
        <div>
          <Field
            ...
            onChange={(value) => setContenuAmpoule(Number(value))}
            suffix="mg"
          />
          <Field
            label="Volume ampoule"
            value={volumeAmpoule}
            onChange={(value) => setVolumeAmpoule(Number(value))}
            suffix="mL"
            step="0.1"
          />
          <Field
            ...
            value={doseSouhaitee}
            onChange={(value) => setDoseSouhaitee(Number(value))}
            suffix="mg"
          />
      ...
      <div className="text-xs text-slate-500 mt-3">
        Toujours réaliser un double contrôle selon les procédures locales.
      </div>
    </Card>
  );
}
  ...
  return (
    <Card title="Débit d'infusion" subtitle="Calcul du mL/h">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Heures" value={heures} onChange={(value) => setHeures(Number(value))} suffix="h" />
        <Field
          label="Minutes"
          value={minutes}
...
function DripRate() {
  ...
  return (
    <Card
      title="Gouttes par minute"
      subtitle="Formule: (Volume × facteur de chute) ÷ temps"
    >
      <Field label="Volume" value={volume} onChange={setVolume} suffix="mL" />
      <Field label="Volume" value={volume} onChange={(value) => setVolume(Number(value))} suffix="mL" />
      <Field label="Temps" value={minutes} onChange={(value) => setMinutes(Number(value))} suffix="min" />
      <Field
        label="Facteur de chute"
        value={df}
        onChange={(value) => setDf(Number(value))}
        suffix="gtt/mL"
      />
...
function GCS() {
  ...
  return (
    <Card
      ...
      subtitle="Additionner Œil (4) + Verbal (5) + Moteur (6)"
    >
      <Select
        label="Ouverture des yeux"
        value={eye}
        onChange={(value) => setEye(Number(value))}
        options={[
          { v: 4, l: 'Spontanée (4)' },
          { v: 3, l: 'À la voix (3)' },
      ...
      <Select
        ...
        value={verbal}
        onChange={(value) => setVerbal(Number(value))}
        options={[
          { v: 5, l: 'Orientée (5)' },
          { v: 4, l: 'Confuse (4)' },
      ...
      <Select
        ...
        value={motor}
        onChange={(value) => setMotor(Number(value))}
        options={[
          { v: 6, l: 'Obéit (6)' },
      ...
      <div className="text-[11px] text-slate-500 mt-2">
        Référence: E4 V5 M6, échelle de Glasgow.
      </div>
    </Card>
  );
}
...
function NEWS2() {
  ...
  const scores = useMemo(() => {
    ...
    if (rr <= 8) s.rr = 3;
    else if (rr <= 11) s.rr = 1;
    else if (rr <= 20) s.rr = 0;
    else if (rr <= 24) s.rr = 2;
    else s.rr = 3;
    if (spo2 <= 91) s.spo2 = 3;
    else if (spo2 <= 93) s.spo2 = 2;
    else if (spo2 <= 95) s.spo2 = 1;
    else s.spo2 = 0;
    s.o2 = o2 ? 2 : 0;
    if (sbp <= 90) s.sbp = 3;
    else if (sbp <= 100) s.sbp = 2;
    else if (sbp <= 110) s.sbp = 1;
    else if (sbp <= 219) s.sbp = 0;
    else s.sbp = 3;
    if (hr <= 40) s.hr = 3;
    else if (hr <= 50) s.hr = 1;
    else if (hr <= 90) s.hr = 0;
    else if (hr <= 110) s.hr = 1;
    else if (hr <= 130) s.hr = 2;
    else s.hr = 3;
    if (temp <= 35.0) s.temp = 3;
    else if (temp <= 36.0) s.temp = 1;
    else if (temp <= 38.0) s.temp = 0;
    else if (temp <= 39.0) s.temp = 1;
    else s.temp = 2;
    s.avpu = avpu === 'A' ? 0 : 3;
  ...
  return (
    <Card title="NEWS2 (adultes)" subtitle="Score d'alerte précoce">
      <div className="grid grid-cols-2 gap-3">
        <Field label="FR" value={rr} onChange={(value) => setRr(Number(value))} suffix="/min" />
        <Field label="SpO₂" value={spo2} onChange={(value) => setSpo2(Number(value))} suffix="%" />
        <Toggle label="Oxygène" checked={o2} onChange={setO2} />
        <Field label="PAS" value={sbp} onChange={(value) => setSbp(Number(value))} suffix="mmHg" />
        <Field label="FC" value={hr} onChange={(value) => setHr(Number(value))} suffix="/min" />
        <Field
          label="Température"
          value={temp}
          onChange={(value) => setTemp(Number(value))}
          suffix="°C"
          step="0.1"
        />
        <Select
          label="Conscience (AVPU)"
          value={avpu}
          onChange={setAvpu}
          options={[
            { v: 'A', l: 'Alerte (A)' },
      ...
      <div className="text-[11px] text-slate-500 mt-2">
        Référence: NEWS2 (Royal College of Physicians). Utilisation selon
        protocole local.
      </div>
    </Card>
  );
...
function CrCl() {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        <Field label="Âge" value={age} onChange={(value) => setAge(Number(value))} suffix="ans" />
        <Field label="Poids" value={poids} onChange={(value) => setPoids(Number(value))} suffix="kg" />
        <Select
          label="Sexe"
          value={sexe}
          onChange={setSexe}
        ...
        <Field
          ...
          value={scr}
          onChange={(value) => setScr(Number(value))}
        />
      </div>
      <Result tone="info">
...
function BMI() {
  ...
  return (
    <Card title="IMC" subtitle="Indice de masse corporelle">
      <div className="grid grid-cols-2 gap-3">
        <Field label="Taille" value={taille} onChange={(value) => setTaille(Number(value))} suffix="cm" />
        <Field label="Poids" value={poids} onChange={(value) => setPoids(Number(value))} suffix="kg" />
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <MiniField label="Poids" value={w} suffix="kg" onChange={(value) => setW(Number(value))} />
        <MiniField label="Dose" value={d} suffix="mg/kg" onChange={(value) => setD(Number(value))} />
        <MiniField
          label="Concentration"
          value={c}
          suffix="mg/mL"
      ...
      <div className="rounded-xl border bg-slate-50 text-slate-800 px-3 py-2 text-sm">
        ≈ {round(ml)} mL
      </div>
    </div>
  );
    ...
    <footer className="mt-10 border-t bg-white/70 backdrop-blur">
      <div className="mx-auto max-w-6xl px-4 py-6 text-sm text-slate-500">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div>
            Conçu pour faciliter le quotidien infirmier. ⚠️ Aide au calcul
            uniquement.
          </div>
          <div>© {new Date().getFullYear()} — Fait avec ❤️</div>
        </div>
      </div>
    </footer>
  );
